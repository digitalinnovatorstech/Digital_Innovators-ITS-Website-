...
const PrimaryButton = ({ text = "Read More", onClick, className = "" }) => {
  return (
    <button
      onClick={onClick}
      className={`relative w-46  h-10 inline-flex items-center justify-center px-6 py-3 rounded-full bg-black text-white text-sm font-medium overflow-hidden group transition-all duration-300 hover:bg-orange-500 ${className}`}
    >
      {/* Initial state */}
